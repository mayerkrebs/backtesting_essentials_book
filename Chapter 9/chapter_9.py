...
class MonteCarloStrategy(Strategy):
    def init(self):
        self.EXPOSURE_PCT = 0.5
 
    def on_bar(self):
        self.ticker = self.tickers[0]
        if random() > self.EXPOSURE_PCT:
                self.rebalance({self.ticker: 0})
        else:
            self.rebalance({self.ticker: 1})
...
class Montecarlo():
    def __init__(self,
        ...
        self.max_exposure_error = max_exposure_error
        self.num_iterations = num_iterations
        self.initial_cash = initial_cash
 
    def __prepare_backtest(self):   
        e = Engine(
            initial_cash=self.initial_cash,
        )
        e.strategy = MonteCarloStrategy()
        e.strategy.EXPOSURE_PCT = self.exposure_pct
        e.add_data(self.asset_prices, 'asset_prices')
 
        self.engine = e
 
    def __run_simulation(self, engine):
        out = engine.run()
        return out
    ...
    def run(self):
        results = []
        while True:
            self.__prepare_backtest()
            statistics = self.__run_simulation(self.engine)
            iter_exposure = statistics['exposure']
            if (abs(
                iter_exposure / 100 - self.exposure_pct)
                < self.max_exposure_error):     
                results.append(statistics)

            if len(results) >= self.num_iterations:
                break
